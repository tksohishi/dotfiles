autocmd BufWinEnter		<buffer>	:call <SID>ScanWholeBuffer()
autocmd FileType		<buffer>	:call <SID>ScanWholeBuffer()
inoremap <ESC> <ESC>:call <SID>ScanLineForColorDefs(line('.'))<CR>
inoremap <C-C> <C-C>:call <SID>ScanLineForColorDefs(line('.'))<CR>
inoremap <CR>  <ESC>:call <SID>ScanLineForColorDefs(line('.'))<CR>a<C-J>
...
function! s:AddColorSyntax(color)
	...
	let syntax = 'syntax match color' . color . ' "' . a:color . '" containedin=cssColor'
	if &filetype =~ 'html'
		let syntax .= ',htmlString'
	endif
	execute syntax
