...
- Never set `"sandbox": {"enabled": true}` in `dotclaude/settings.json`. Enabling the bash sandbox also forces all command network traffic through a filtering proxy (breaks SSH `git push`, adds session-wide friction) and takes effect mid-session. Secrets-file protection stays at the hook layer (`bash-antipatterns.sh`) instead. If revisited on explicit request: sandbox globs need leading `/**/` (relative `**/` resolves against the settings dir), deny beats allow so enumerate `.env` variants rather than `.env.*` to keep `.env.example` writable, and `allowRead` for secrets files is required or bun hard-fails in any directory containing `.env`; last working config is at commit d75a6d0.
