...
- `slk` prints timestamps in MACHINE-LOCAL time (`YYYY-MM-DD HH:MM`, no zone suffix) — verified against a raw `ts` with `date -r <ts>`. When precision matters (cross-referencing commit times, ordering events), don't assume: confirm once per session by comparing a message's `ts` (from `--json`) against its displayed time. Label the zone when quoting times back to the user.
