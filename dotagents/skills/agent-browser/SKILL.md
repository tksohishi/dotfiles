...
- **Waits: human typing speed.** Between form interactions (type, click a dropdown, pick a date) `wait 1000` is enough; the page is already loaded and the DOM settles in well under a second. Reserve 5-15s waits for navigations, a submit that triggers a search, and challenge pages. A form driven with 10-15s dwells took 8+ minutes for one search (qantas.com, 2026-09-02).
...
- One continuous session. Every open and close of a headed window grabs keyboard focus from whatever the user is typing in, so a headed flow is planned as a single launch: verify launch args and code paths headless first, never do a test launch before the real one, never close and reopen to "fix" state, and once the window is up only read from it (`get text`, `snapshot`) while the user drives, until they say the flow is done (Sep 8, 2026: a launcher crashed on a launch-flag mismatch and was relaunched, two focus grabs in a minute).
