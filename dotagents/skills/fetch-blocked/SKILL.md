...
- Ready-made runner on PATH: `patchright-fetch <url> [--wait <seconds>] [--show]` (dotfiles bin; bun auto-installs pinned patchright on first run, needs Chrome Canary). Per-host persistent profile under cwd-local `tmp/patchright/`, default 20s wait (deliberately long; bot walls need settle time — shorten only when the site is known fast), body text saved to `tmp/patchright/<host>.txt`. Prefer it over writing a new script.
