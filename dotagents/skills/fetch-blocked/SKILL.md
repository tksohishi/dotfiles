...
- **One-time manual solve walls (PerimeterX Press & Hold, DataDome slider).** Recognize them early: a PX "Robot or human?"/"Press & Hold" page, or a DataDome stub (`var dd={...}`, `geo.captcha-delivery.com`, `Server: DataDome`). On a fresh profile the offscreen run cannot pass these and it is not a failed ladder: tell the user which host wants a challenge solved, wait for them to say they are ready, run `patchright-fetch <url> --show --wait 60`, and let them solve it in that window. The clearance lives in the per-host profile, so later offscreen runs pass without the user until the site's trust expires or the IP/Chrome version changes. Verified PX: StreetEasy 2026-08-31. Verified DataDome: yelp.com 2026-09-08 (the user solved a slider once; that run's report of "no captcha rendered" was wrong). Do not record such a host as "no verified path" until `--show` with a manual solve has been tried (vitaminshoppe.com and gnc.com are in that state).
