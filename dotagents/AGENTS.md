...
- Delegate to a subagent when it keeps bulk *out* of the main context — image and screenshot reads, wide multi-file searches, multi-source research (10+ URLs or cross-source comparison), long tool output you only need a summary of. Ask for a bounded summary ("under 300 words"). Integrate subagent results from their artifacts (diffs, files on disk, command output), not their closing summary — an agent's final report is the least reliable part of its trajectory; failed runs routinely assert compliance. Don't delegate what you could finish in a few tool calls: overhead is ~10x a direct fetch for a simple lookup. `CLAUDE_CODE_SUBAGENT_MODEL` pins subagents to Opus regardless of the main model, so when the main session runs Fable, delegating bulk work also conserves Fable quota. Don't pass a `model` param; it's overridden.
