...
- Within the current project, prefer relative paths — for git (`git add foo.ts`), scripts (`bun scripts/foo.ts`, `python scripts/foo.py`, `./bin/foo`), and file ops (`ls src/`, `cat config.toml`). Use absolute paths only for files outside the project or when wd is genuinely ambiguous. Never chain `cd X && <tool> <relative path>` (rg, grep, cat, sed, head, tail, fd, ls, find): the Read() deny rules make Claude Code escalate any file argument it can't resolve after a `cd` to a permission prompt, and a hook blocks the shape. Replace with `rg <pattern> /abs/dir/file`, `cat /abs/dir/file`, `ls /abs/dir`, or `git -C /abs/dir <cmd>`.
