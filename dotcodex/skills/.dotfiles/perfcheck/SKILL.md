---
name: "perfcheck"
description: "Run a quick machine performance check (load, CPU, memory, thermal)"
---

Use this skill when the user asks to run `/perfcheck`.


Check machine performance. Run these commands as **separate** Bash calls (not chained) so each result is reviewable on its own:
...
- **Memory:** flag if compressor pages are large or swapouts are active now (not just lifetime)
