...
2. `HOMEBREW_BUNDLE_MAS_SKIP="$(rg -o 'id: [0-9]+' Brewfile | rg -o '[0-9]+' | tr '\n' ' ')" brew bundle install --no-upgrade` — install any missing Brewfile entries without upgrading already-present ones (upgrades are handled by steps 3-4 for formulae/casks, auto-update for GUI casks, and `mas upgrade` manually for App Store apps). The env var skips every `mas` entry: mas detects installed apps via Spotlight and sporadically misses some (2026-08-26: Day One, Amphetamine, CotEditor), which makes bundle run `mas install` on already-installed apps, and that path calls sudo and fails headlessly. App Store installs need a password anyway, so this step can never usefully install them; `install.sh` (interactive) still handles them on a fresh machine. `brew bundle` has no blanket mas-skip flag, hence the ID list.
3. `brew upgrade --formula` — upgrade all formulae
4. `brew upgrade --cask gcloud-cli $(brew info --json=v2 --installed | jq -r '.casks[] | select((.auto_updates | not) and (.version != "latest")) | .token')` — upgrades only casks with no self-update mechanism (CLI casks like codex, 1password-cli, notion-cli; versioned fonts; libreoffice). `gcloud-cli` is listed by name because Homebrew marks it `auto_updates` (its self-update is `gcloud components update`), yet it is a CLI with no Gatekeeper concern, so leaving it to the filter just left it stale. Self-updating GUI casks (Cursor, VS Code, ChatGPT, ...) are deliberately excluded: a brew re-download re-quarantines the bundle, triggering a one-time Gatekeeper "downloaded from the internet" dialog on next launch, whereas in-app self-update is silent. The cost is that a self-updating app you never launch stays stale until you open it. `version :latest` casks (font-sf-pro, font-sf-mono-nerd-font-ligaturized) are also excluded: brew "upgrades" them by re-downloading and uninstalling the previous artifact, and for pkg-based ones (font-sf-pro) that uninstall runs sudo, which fails headlessly and purges the cask record while leaving files on disk. Reinstall those manually with `brew reinstall --cask <name>` when needed. Never add `--greedy` or run a bare `brew upgrade --cask`.
