...
4. `brew upgrade --cask $(brew info --json=v2 --installed | jq -r '.casks[] | select(.auto_updates | not) | .token')` — upgrades only casks with no self-update mechanism (CLI casks like codex, 1password-cli, notion-cli; fonts; libreoffice). Self-updating GUI casks (Cursor, VS Code, ChatGPT, ...) are deliberately excluded: a brew re-download re-quarantines the bundle, triggering a one-time Gatekeeper "downloaded from the internet" dialog on next launch, whereas in-app self-update is silent. The cost is that a self-updating app you never launch stays stale until you open it. Never add `--greedy` or run a bare `brew upgrade --cask`. Warn in the report that upgraded GUI apps may need a relaunch if they were running
