...
2. Install missing Brewfile entries — but NEVER via `brew bundle install`. Homebrew has a load-order bug (present in 6.0.x as of 2026-08-19, unreported upstream at the time): when the Brewfile contains any `brew` formula entry, bundle's cask detection sees ZERO installed casks, so `brew bundle install` (even with `--no-upgrade`) re-downloads and reinstalls every cask — re-quarantining self-updating GUI apps (one-time Gatekeeper dialog on next launch), attempting sudo-gated upgrades that fail headlessly, and purging `version :latest` pkg-cask records (font-sf-pro). Instead:
   - Run `brew bundle check --verbose` and collect the reported-missing entries. Treat cask results as untrusted because of the bug above.
   - Cross-check every reported-missing cask with `brew list --cask --versions <tokens>`; drop the ones actually installed. Formula results are reliable, but cheap to confirm with `brew list --versions <names>`.
   - Install only the genuinely missing entries individually: `brew install <formula>` / `brew install --cask <token>`. Missing `mas` apps are reported by the Mac App Store subagent; skip them here.
   - If nothing is genuinely missing (the usual case), skip installation entirely.
   (Upgrades are handled by steps 3-4 for formulae/casks, auto-update for GUI casks, and `mas upgrade` manually for App Store apps.)
